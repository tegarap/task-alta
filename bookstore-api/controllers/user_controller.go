...
func CreateUserController(c echo.Context) error {
	...
	if err != nil {
		return echo.NewHTTPError(http.StatusBadRequest, common.ResponseFail("failed add user"))
	}
...
func GetUserDetailControllers(c echo.Context) error {
	...
	if loggedInUserId != id {
		return echo.NewHTTPError(http.StatusUnauthorized, common.ResponseFail("unauthorized access"))
	}

	users, err := database.GetDetailUsers(id)
	if err != nil {
		return echo.NewHTTPError(http.StatusBadRequest, common.ResponseFail("cannot get detail user"))
	}
